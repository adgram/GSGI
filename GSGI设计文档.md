...
| `tags` | string[] | 否 | 文档分类标签，建议 2–6 个，从小范围到大范围排列 |
...
| 角度 | 以弧度（radian）为单位，从 X 轴正方向起，**逆时针**为正（text 的 rotation 例外，使用度） |
...
| `description` | string | 否 | — | 图层自然语言说明，供 AI 快速理解图层语义 |

### 文本样式

文本样式定义文档中可用的文字样式，供 `text`、`table` 等实体引用。
...
| 字段 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| `point` | `[number, number]` | 是 | — | 坐标 `[x, y]` |
| `ref_pt` | `string \| object` | 否 | — | 引用另一实体。string 形式为被引用实体 id（向后兼容）；object 形式 `{ "id": "CS1", "represent": "center", "ref_op": "link" }`，其中 `id` 为被引用实体 id（必填），`represent` 和 `ref_op` 可选，用于覆盖被引用实体的对应字段。省略则使用被引用实体自身的值 |
| `point_role` | `string` | 否 | — | 语义角色，可选值：`"construction"`（构造点）、`"helper"`（辅助点）。不设则为普通点。 |
...
2. 有 `ref_pt`（string 形式）：解析被引用实体 B 的代表点 P_rep（按 B 的 `represent`），再按 B 的 `ref_op` 组合。
3. 有 `ref_pt`（object 形式）：解析被引用实体 B 的代表点 P_rep（优先按 `ref_pt.represent`，否则按 B 自身 `represent`），再按 `ref_pt.ref_op`（优先）或 B 自身 `ref_op` 组合。

**引用类型限制**：当被引用实体 B 的类型为 `point` 或 `param_pt` 时，其 `represent` 固定为 `"self"`、`ref_op` 固定为 `"offset"`。因此在 `ref_pt` object 中，`represent` 仅允许 `"self"`、`ref_op` 仅允许 `"offset"`，其他值视为无效（静默回退到被引用实体自身的值）。
...
| `arc` | `[0, 1]` | 沿弧长比例插值，`t=0` = start_ref 点，`t=1` = end_ref 点（对圆弧，弧长比等于圆心角比） |
...
**bulge 转弧中点（polyarc 转 3P arc 转换）**：
- 弦中点 `C = ((Sx+Ex)/2, (Sy+Ey)/2)`
- 垂直距离 `d = chord/2 · (1−b²) / (2·b)`，其中 `b = bulge`；等价于 `d = r − cos(θ/2)·r`
- 弧中点 `M = C + d · normalize(perp(S→E))`，`perp((dx,dy)) = (−dy, dx)` 当 `b>0`（逆时针），`perp((dx,dy)) = (dy, −dx)` 当 `b<0`（顺时针）
...
| `arc` | `start_ref`, `mid_ref`, `end_ref` | 圆弧段（三点定义，三点唯一确定一段圆弧） |
...
**语义约定**：各段按顺序首尾相接，line 段的 `start_ref` 须等于前段的 `end_ref`，arc 段的 `start_ref` 须等于前段的 `end_ref`、`end_ref` 须等于后段的 `start_ref`（验证规则：`segments[i].end_ref == segments[i+1].start_ref`）。
...
|------|------|--------|------|
| `start_ref` | 是 | — | 起点引用的 point 实体 id |
| `mid_ref` | 是 | — | 弧上经过的中间点 point 引用（唯一确定弧的弯曲方向） |
| `end_ref` | 是 | — | 终点引用的 point 实体 id |
| `represent` | 否 | `"center"` | 弧曲线参数 `t=0.5` 处的点 |

**数学基础**：三点非共线时唯一确定一个外接圆圆弧。`param_pt.t` 语义保持 `[0, 1]` 归一化，`t=0` = 起点、`t=1` = 终点，沿弧长线性映射（对圆弧而言弧长比等于圆心角比）。
...
| `scale_x` | 否 | `1` | X 缩放比例 |
| `scale_y` | 否 | `1` | Y 缩放比例 |
| `attrs` | 否 | — | 属性覆盖值，实例化时传入 |
| `represent` | 否 | `"base"` | 基点位（插入点） |
| `ref_op` | 否 | `"offset"` | 被引用时偏移叠加 |

**多行文本**：使用 `\n` 换行。DXF 导出时：含 `\n` 写入 `MTEXT`，不含 `\n` 写入 `TEXT`。

### 5.12 spline_fit（拟合点曲线）
...
用 markdown 表格语法简化表示。单元格支持前缀标记表达非文本内容和单元格引用：

| 前缀 | 含义 | 示例 |
|------|------|------|
| 无前缀 | 纯文本 | `角钢` |
| `@实体ID` | block_ref — 显示引用图块 | `@BR1` |
| `%实体ID` | xref — 显示引用外部参照 | `%XR1` |
| `^R行C列` | 单元格引用 — 显示另一格内容（行、列均 0-based） | `^R0C0` |
...
| `markdown` | 是 | — | markdown 表格语法，支持前缀标记；`^R{r}C{c}` 为单元格引用 |
| `represent` | 否 | `"base"` | 基点位（表格左上角） |
| `ref_op` | 否 | `"offset"` | 被引用时偏移叠加 |

**单元格引用规则**：
- `^R{r}C{c}` 引用同一表格内另一格的内容（行 r、列 c，0-based）
- 支持任意形状的跨格引用（不限于矩形合并）
- 引用链递归解析；循环引用必须由实现检测并报错
- **`merges` 字段已移除**——所有跨格合并统一由 `^R…C…` 表达
...
| `height` | 否 | `12` | `area_text` 的文字高度，受 `scale` 系数缩放 |
| `contained_entities` | 否 | — | 区域内包含的实体 id 列表 |
| `label` | 否 | — | 区域标签 |
| `fill` | 否 | — | 填充图案（详见 [fill 对象结构](#fill-对象结构)）——存在时本 region_anno 起到填充区域的作用 |
...
- 当 `fill` 存在时，region_anno 额外定义了一个视觉填充区域——边界模型（`edges_refs`）同时服务于区域语义和填充几何
- DXF HATCH 映射为 `region_anno` + `fill`
...
当 `fill` 存在时，region_anno 成为视觉填充区域。边界几何由 `edges_refs` 定义（与区域语义共享）。

| 子字段 | 类型 | 必填 | 默认值 | 说明 |
|--------|------|------|--------|------|
| `pattern` | string | 是 | — | 填充图案名；`"SOLID"` 为实心填充，其他如 `"ANSI31"`、`"AR-CONC"` |
| `color` | int/string | 否 | 继承 | 填充颜色（ACI 索引或 hex RGB），详见 [颜色系统](#颜色系统) |
| `angle` | number | 否 | `0` | 图案旋转角度（弧度） |
| `scale` | number | 否 | `1.0` | 图案缩放密度 |

**语义约定**：
- `fill` 不存在或为 `null` → region_anno 为纯语义区域（无视觉填充）
- `fill` 存在 → region_anno 额外定义填充区域，导出时映射为 DXF `HATCH`
- 多个不相连的填充区域用多条 `region_anno` 表达
...
| `attributes` | object | 否 | 声明的参数列表，用于参数化图块实例化 |
...
**约定**：变换矩阵以实体 `represent` 代表点为变换中心。如需绕其他点变换，需先平移到代表点，再应用变换，最后平移回原位置（组合入同一矩阵）。
...
| 实体类型 | 计算方式 |
|---------|---------|
| line, spline_fit, spline_cv, dimension | 两端点／首尾参数点的中点 |
...
| method | 参数 | 含义 | 公式 |
|--------|------|------|------|
| `"corner"` | `param: int` (0‑based) | 取实体定义序列中的第 param 个特征点：line/arc 0=起点 1=终点；polyline/polyarc 0..(n-1)=顶点；spline_fit/spline_cv 0..(n-1)=拟合/控制点；polycurve 0..(n-1)=段端点；rectangle 0=min 1=min→max 2=max 3=max→min | `P = entity.pointAt(param)` |
...
| `"param"` | `t: number` | 沿自身曲线在参数 `t` 处取点。`t` 语义同 [§5.2 param_pt](#52-param_pt曲线参数点)：line/arc/spline_* 归一化 `[0, 1]`；polyline/polyarc 开放 `[0, N]` 闭合 `[0, N)`。无 `getCurve()` 的实体（text/block_ref 等）回退 `"origin"` | `P = curve.eval(t)` |
...
**区域标注**既是实体类型（见 [§5.17](#517-region_anno区域标注)），也是描述系统的一部分。在描述层面，它用于声明一块区域的语义概念（轮廓、面积、标签及关联实体）。它不产生新几何线条，仅引用已有几何边界来定义区域。
...
| `label` | string | 否 | — | 区域标签 |
| `fill` | object | 否 | — | 填充图案定义（详见 §5.17 fill 对象结构）；存在时本 region_anno 同时作为 DXF HATCH 的等价物 |
| `description` | string | 否 | — | 自然语言描述 |
| `operation` | object | 否 | — | 标记该区域是由布尔运算派生得来；详见 §5.17 `operation` 对象结构 |
