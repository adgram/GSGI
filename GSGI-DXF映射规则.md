...
| 角度单位 | 除 text/mtext 外，GSGI 角度字段使用弧度；DXF 使用度 | GSGI→DXF：弧度→度；DXF→GSGI：度→弧度 |
...
| polycurve | LWPOLYLINE/SPLINE | 按 segment 类型拆分：`line`→LINE, `arc`→ARC, `subsegment_ref`→LWPOLYLINE, `curve_ref`→SPLINE/LINE | 或降级为采样 LWPOLYLINE |
| mline | MLINE | 多线顶点展开为点坐标；元素属性映射 | 暂无 |
| circle | CIRCLE | `10,20`=圆心；`40`=半径 | center_ref 解析 |
| arc | ARC | `10,20`=圆心；`40`=半径；`50`=起角；`51`=终角 | 角度以弧度→度 |
| rectangle | LWPOLYLINE | 4 顶点，`70`=1（闭合） | min_ref/max_ref 展开 |
| text | TEXT | `10,20`=插入点；`1`=内容；`40`=字高；`50`=旋转 | position_ref 解析 |
| mtext | MTEXT | `10,20`=插入点；`1`=内容；`40`=字高；`41`=宽度；`50`=旋转；`71`=附着点 | position_ref 解析 |
...
| hatch | HATCH | `91`=边界数；`52`=图案角度；边界由 `outer_ref`/`island_refs` 引用的 polycurve 展开 | polycurve 须采样为闭合环；图案角度弧度→度 |
| subsegment | LWPOLYLINE | 沿母曲线采样为多段线 | 采样密度由实现定义 |
| dimension | DIMENSION | `13,23`=定义点；`14,24`=文本点 | p1_ref/p2_ref 解析 |
| region_anno | （描述系统，无独立 DXF 实体） | 边界由 edges_refs 引用的 polycurve 映射为 DXF 中的闭合 LWPOLYLINE/HATCH；面积/标签/关联实体存入 XDATA `GSGI_REGION` | region_anno 在 GSGI 中既是实体也属描述系统 |
| position | 无实体 | 全部存入 XDATA `GSGI_POS` | ref_a/ref_b 存为 id |
| measure | 无实体 | 全部存入 XDATA（同 position） | 不可见实体 |
...
| MLINE | mline | 生成 point 数组 + elements |
| CIRCLE | circle | 生成 center point |
| ARC | arc | 生成 center point |
| TEXT | text | 生成 position point |
| MTEXT | mtext | 生成 position point |
...
| HATCH（有填充） | hatch | 边界环生成 polycurve；outer_ref + island_refs |
| HATCH（无填充）+ XDATA `GSGI_REGION` | region_anno | 边界环生成 polycurve；面积/标签从 XDATA 还原 |
...
| —（XDATA） | measure | 从 XDATA 还原（同 position） |
...
| `GSGI_REGION` | region_anno 全量 JSON `{"area":0,"label":"","contained_entities":[],"operation":{}}` | 边界由 polycurve 实体映射 |
...
2. **polycurve 采样**：HATCH/REGION_ANNO 边界引用 polycurve，DXF 输出时须将 polycurve 采样为闭合 LWPOLYLINE 顶点序列
